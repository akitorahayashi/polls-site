...
.PHONY: setup
setup: ## .envがない場合、.env.exampleから.envファイルを作成
...
.PHONY: up
up: ## Dockerイメージをビルドし、コンテナをバックグラウンドで起動
	@echo "Building images and starting containers..."
	@docker compose up --build -d

.PHONY: down
down: ## 実行中のコンテナを停止し、孤立コンテナを削除
...
makemigrations: ensure-web ## モデル変更時にマイグレーションファイルを作成
